...
class TestGrid(ModelGrid):
    def __init__(self):
        ...
        self.params['Fe_H'] = ModelParam('Fe_H', np.array([0, 1, 2]))
        self.params['T_eff'] = ModelParam('T_eff', np.array([1, 2]))
        self.params['log_g'] = ModelParam('log_g', np.array([0, 5.5, 0.5]))
...
class TestModelGridSpectrumReader(TestBase):
    ...
    def test_read_grid(self):
        g = TestGrid()
        g.init_storage(np.linspace(3000, 6000, 1))
        r = TestGridReader(g)
        r.read_grid()