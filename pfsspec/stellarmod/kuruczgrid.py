...
class KuruczGrid(ModelGrid):
    def __init__(self, model='kurucz', use_cont=True):
        super(KuruczGrid, self).__init__(use_cont=use_cont)

        if model == 'test':
            self.params['Fe_H'] = ModelParam('Fe_H', np.array([0.0, 0.1]))
            self.params['T_eff'] = ModelParam('T_eff',
                         np.array([3500., 3750., 4000., 4250., 4500., 4750., 5000., 5250., 5500.,
                                   ...
                                   47500., 50000.]))
            self.params['log_g'] = ModelParam('log_g', np.arange(0, 5.1, 0.5))
        elif model == 'kurucz':
            self.params['Fe_H'] = ModelParam('Fe_H',
